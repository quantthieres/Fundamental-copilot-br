import type { BankFinancialRecord, BankIndicators } from "./bank-types";
...
function growthRate(current: number | null | undefined, prior: number | null | undefined): number | null {
...
export function computeBankIndicators(annual: BankFinancialRecord[]): BankIndicators | null {
  ...
  const loanToAssets    = latest.loanPortfolio != null
    ? safeDiv(latest.loanPortfolio, latest.totalAssets)
    : null;
  const depositsToAssets = latest.deposits != null
    ? safeDiv(latest.deposits, latest.totalAssets)
    : null;
